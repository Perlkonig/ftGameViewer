<script lang="ts">
    ...
    const outerMove = (e: MouseEvent) => {
        ...
        mouseX = currMouse.x / pixelsPerMU;
        mouseY = currMouse.y / pixelsPerMU;
...
<main class="container p-6">
    <div>
        <div class="field is-grouped">
            <div class="control">
                <input class="input" name="mouseX" type="number" bind:value={mouseX}>
            </div>
            <div class="control">
                <input class="input" name="mouseY" type="number" bind:value={mouseY}>
            </div>
            <div class="control">
                <input class="input" name="circleID" type="text" bind:value={circleID}>
            </div>
        </div>
    </div>
    <div class="mapSvg">
    {#if circles === undefined}
        <p>Generating circles...</p>
    {:else}
        <svg bind:this="{outerSvg}" viewBox="-1 -1 {maxX + (rulerWidth * 2) + (rulerGap * 2) + 2} {maxY + (rulerWidth * 2) + (rulerGap * 2)  + 2}" on:mousemove="{outerMove}">
            ...
            <!-- And here's the map -->
            <svg bind:this="{innerSvg}" viewBox="{$currX} {$currY} {$currWidth} {$currHeight}" x="{rulerWidth + rulerGap}" y="{rulerWidth + rulerGap}" width="{maxX}" height="{maxY}" on:wheel="{handleWheel}" on:mousedown="{dragStart}" on:mouseup="{dragEnd}" on:mouseleave="{dragEnd}" on:mousemove="{dragMouse}">
                <rect id="_background" x="0" y="0" width="{maxX}" height="{maxY}" fill="black" />
            {#each circles as c}
                <circle id="{c.id}" cx="{c.cx}" cy="{c.cy}" r="{c.r}" fill="rgb({c.red}, {c.green}, {c.blue})" on:click={() => circleID = c.id}/>
            {/each}
            </svg>
        </svg>
    {/if}
