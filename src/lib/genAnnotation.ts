import type { IAnnotation, IPoint, IArc, ICircle, ILine, IPoly } from "@/stores/writeAnnotations";
...
const generateSVG = (a: IAnnotation, defaultColour: string, ppmu: number): string => {
    if (a.type === "CIRCLE") {
        ...
        return genPoly(a, defaultColour, ppmu);
    }
    return "";
}
...
const genCircle = (a: IAnnotation, defaultColour: string, ppmu: number): string => {
    ...
    if (startR === undefined) {
        return `<circle id="_annotation_${a.id}" cx="${cx}" cy="${cy}" r="${r}" fill="${colour}" fill-opacity="${opacity}" stroke="${colour}" stroke-width="${width}" pointer-events="none" />`
    } else {
        return `<path d="M ${cx},${cy} m 0,-${r} a ${r},${r},0,1,0,1,0 Z m 0,${r - startR} a ${startR},${startR},0,1,1,-1,0 Z" fill="${colour}" fill-opacity="${opacity}" stroke="${colour}" stroke-width="${width}" pointer-events="none" />`;
    }
}
...
const genArc = (a: IAnnotation, defaultColour: string, ppmu: number): string => {
    ...
    let path: string;
    if (startR > 0) {
        path = `<path d="M ${leftOuter.x} ${leftOuter.y} A ${r} ${r} 0 ${large} 1 ${rightOuter.x} ${rightOuter.y} L ${rightInner.x} ${rightInner.y} A ${startR} ${startR} 0 ${large} 0 ${leftInner.x} ${leftInner.y} Z" fill="${colour}" fill-opacity="${opacity}" stroke="${colour}" stroke-width="${width}" pointer-events="none" />`;
    } else {
        path = `<path d="M ${cx} ${cy} L ${leftOuter.x} ${leftOuter.y} A ${r} ${r} 0 ${large} 1 ${rightOuter.x} ${rightOuter.y} Z" fill="${colour}" fill-opacity="${opacity}" stroke="${colour}" stroke-width="${width}" pointer-events="none" />`;
    }
    return path;
}
...
const genLine = (a: IAnnotation, defaultColour: string, ppmu: number): string => {
    ...
    return `<line id="_annotation_${a.id}" x1="${x1*ppmu}" y1="${y1*ppmu}" x2="${x2*ppmu}" y2="${y2*ppmu}" fill="${colour}" fill-opacity="${opacity}" stroke="${colour}" stroke-width="${width}" stroke-opacity="${opacity}" pointer-events="none" />`;
}

const genPoly = (a: IAnnotation, defaultColour: string, ppmu: number): string => {
    const note = a.note as IPoly;
    let colour = defaultColour;
    if (a.color !== undefined) colour = a.color;
    let opacity = 1;
    if (a.opacity !== undefined) opacity = a.opacity;
    let width = 5;
    if (a.strokeWidth !== undefined) width = a.strokeWidth;
    const pts = note.points.map((p) => `${p.x * ppmu},${p.y * ppmu}`).join(" ");
    return `<polyline id="_annotation_${a.id}" points="${pts}" fill="none" stroke="${colour}" stroke-width="${width}" stroke-opacity="${opacity}" pointer-events="none" />`;
}
